...
def smooth_loss(mean, logvar):
    mean_t, mean_t_next = mean[:, :-1], mean[:, 1:]
    logvar_t, logvar_t_next = logvar[:, :-1], logvar[:, 1:]

    kl_t = 0.5*(
        (logvar_t_next - logvar_t) + ((torch.exp(logvar_t) + (mean_t - mean_t_next)**2)/torch.exp(logvar_t_next))-1
    ).mean(dim = 1)

    return torch.mean(kl_t)
...
def criterion(mean, logvar, z_mean, z_logvar, x, beta=.5):
    ...
    smooth = smooth_loss(mean, logvar)
    
    # Total_Loss
    loss = recon_loss + beta * kl_div + 0.1*smooth
    return loss
