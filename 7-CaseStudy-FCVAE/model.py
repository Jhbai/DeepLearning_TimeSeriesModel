...
class FCVAE(nn.Module):
    ...
    def anomaly(self, x):
        ...
        with tqdm(total = len(Data), desc = "training") as pbar:
            for data in Data:
                ...
                # res += [self.frequency_loss(data, pred)]
                pbar.update()
        return res
    ...
    def frequency_loss(y_true, y_pred):
        y_true_freq = torch.fft.fft(y_true)
        y_pred_freq = torch.fft.fft(y_pred)
        return torch.mean((torch.abs(y_true_freq) - torch.abs(y_pred_freq))**2)